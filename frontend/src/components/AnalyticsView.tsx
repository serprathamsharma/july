    ...
    <div className="w-full max-w-6xl mx-auto px-4 py-8 animate-fadeIn">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className="glass-panel rounded-2xl p-6 border border-slate-800/80 relative overflow-hidden">
          ...
          <div className="text-4xl font-extrabold text-white font-mono tracking-tight">{latency.p50} <span className="text-lg text-slate-400 font-sans">ms</span></div>
          <span className="text-[11px] text-slate-500 font-mono mt-2 block">Median Execution Time</span>
        </div>
        ...
        <div className="glass-panel rounded-2xl p-6 border border-slate-800/80 relative overflow-hidden">
          ...
          <div className="text-4xl font-extrabold text-white font-mono tracking-tight">{latency.p70} <span className="text-lg text-slate-400 font-sans">ms</span></div>
          <span className="text-[11px] text-slate-500 font-mono mt-2 block">70th Percentile Target</span>
        </div>
        ...
        <div className="glass-panel rounded-2xl p-6 border border-slate-800/80 relative overflow-hidden">
          ...
          <div className="text-4xl font-extrabold text-white font-mono tracking-tight">{latency.p100} <span className="text-lg text-slate-400 font-sans">ms</span></div>
          <span className="text-[11px] text-slate-500 font-mono mt-2 block">Maximum Tail Latency</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
        ...
        <div className="glass-panel rounded-2xl p-6 border border-slate-800">
          ...
          <div className="space-y-4">
            {[
              { label: 'Sarvam STT', value: pipeline_breakdown.stt_ms },
              { label: 'Query Processing', value: pipeline_breakdown.query_processing_ms },
              { label: 'Embedding Generation', value: pipeline_breakdown.embedding_ms },
              { label: 'FAISS Dense Retrieval', value: pipeline_breakdown.dense_retrieval_ms },
              { label: 'BM25 Lexical Search', value: pipeline_breakdown.bm25_ms },
              { label: 'RRF Score Fusion', value: pipeline_breakdown.fusion_ms },
              { label: 'Grounded LLM Generation', value: pipeline_breakdown.generation_ms },
              { label: 'Guardrail Validation', value: pipeline_breakdown.guardrail_ms },
            ].map((stage) => (
              <div key={stage.label}>
          ...
          <div className="mt-6 pt-4 border-t border-slate-800 flex justify-between items-center text-xs font-mono text-slate-400">
            ...
            <span className="text-white font-bold text-sm">{pipeline_breakdown.total_ms} ms</span>
          </div>
        </div>
        ...
        <div className="glass-panel rounded-2xl p-6 border border-slate-800">
          ...
          <div className="grid grid-cols-2 gap-4 mb-6">
            <div className="bg-slate-900/60 p-4 rounded-xl border border-slate-800">
              ...
              <span className="text-2xl font-bold font-mono text-white">{(retrieval.recall_at_5 * 100).toFixed(1)}%</span>
            </div>

            <div className="bg-slate-900/60 p-4 rounded-xl border border-slate-800">
              <span className="text-xs text-slate-400 block mb-1">MRR Score</span>
              <span className="text-2xl font-bold font-mono text-white">{retrieval.mrr}</span>
            </div>
          </div>
          ...
          <div className="space-y-3 text-xs">
            ...
            <div className="flex justify-between py-2 border-b border-slate-800">
              ...
              <span className="font-semibold text-slate-200">{retrieval.active_chunking_strategy}</span>
            </div>
      ...
      <div className="glass-panel rounded-2xl p-6 border border-slate-800">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          <div className="bg-slate-900/60 p-4 rounded-xl border border-slate-800 text-center">
            ...
            <span className="text-2xl font-mono font-bold text-white">{guardrails.queries_rejected}</span>
          </div>

          <div className="bg-slate-900/60 p-4 rounded-xl border border-slate-800 text-center">
            <span className="text-xs text-slate-400 block mb-1">Low-Confidence Abstentions</span>
            <span className="text-2xl font-mono font-bold text-white">{guardrails.low_confidence_abstentions}</span>
          </div>

          <div className="bg-slate-900/60 p-4 rounded-xl border border-slate-800 text-center">
            <span className="text-xs text-slate-400 block mb-1">Grounding Failures Replaced</span>
            <span className="text-2xl font-mono font-bold text-white">{guardrails.grounding_failures}</span>
          </div>

          <div className="bg-slate-900/60 p-4 rounded-xl border border-slate-800 text-center">
            <span className="text-xs text-slate-400 block mb-1">Unsafe Queries Blocked</span>
            <span className="text-2xl font-mono font-bold text-white">{guardrails.unsafe_queries_blocked}</span>
          </div>
        </div>
      </div>
    </div>
